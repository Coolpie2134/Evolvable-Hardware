...
class InteractiveTab:
    ...
    def sync(self):
        ...
        if backend == 'nervous':
            ...
            ttk.Label(self._ctrl, text='   (toggle inputs, then Step/Run to watch pulses & memory)',
    ...
    def _on_toggle(self):
        if not self._circuit:
            return
        if self._circuit['backend'] == 'nervous':
            self._draw_nervous()        # show that inputs changed; takes effect next Step
        else:
            self._eval_snn()

    # ── nervous playback (asynchronous pulse engine, one tick per Step) ───────────
    ...
    def _draw_nervous(self):
        ...
        draw_hex_net(axg, self._grid, gs, routing=self._routing, in_pos=self._in_pos,
                     out_pos=self._out_pos, activity=self._state, show_edges=True,
                     title='tick %d   inputs=%s' % (self._tick, ''.join(map(str, self._in_bits()))))

        # output trace strip
        axt.set_title('output pulse trains', fontsize=9)
        for k, t in enumerate(target.outputs):
            tr = self._trace.get(t.role, [])
            if tr:
                xs = range(len(tr))
                axt.step(xs, [v + k * 1.4 for v in tr], where='post', lw=1.4, label=t.role)
