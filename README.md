...
- **Nervous-net node-timing models** (`nv_evo/pulse.py`, selectable per run via the **Node** dropdown in the pulse-physics row; also `PulseConfig.model` / `GAConfig.node_model`): the paper's node regenerates one fixed pulse width after one fixed delay, so an input pulse's *length* is discarded when the first node fires. Alongside it are two variants that give width a role:
  - **Uniform (paper)** — every node emits width `WIDTH` after delay `DELAY`. The default; unchanged.
  - **Evolved width** — each node *type* (routing state) carries an evolvable pulse-width multiplier (`Genome.state_widths`), so a node's emitted pulse takes that width and a mutation operator tunes it. Pulse width becomes a heritable trait selection can shape (measured: on `Echo` it reached a higher fitness than uniform in the same short budget by discovering non-neutral widths). Delay is unchanged.
  - **Width-preserving + evolved delay** (internal compatibility id `pulse_delay`) — every node transports the complete incoming waveform after its routing-state delay `d_node`: `[t, t+w)` becomes `[t+d_node, t+d_node+w)`. Both edges use the same heritable delay, so width survives each hop while propagation speed evolves. Overlapping or touching drives remain one wired-OR waveform; OR transports their union, while an exact coincidence transports their overlap.
  These are audited in `tests/test_pulse_models.py` (uniform stays byte-identical; evolved-width is unchanged and follows its genetic width map; width-preserving follows its genetic delay map, propagates late extensions, and retains width across multiple hops). Width-sensitive temporal targets now exercise useful relations rather than isolated node contracts: **Pulse width sum (A+B)** emits a Q interval whose duration is `width(A)+width(B)`, while **Odd pulse selector** passes the 1st/3rd/5th input pulses and preserves their individual widths. Both score complete output waveforms (`PulseSim.pulse_intervals`, real rises *and* falls), rather than treating pulses with equal leading edges as equivalent. A related mixed-width stateful family counts every physical pulse once regardless of its 0.5-to-2.25-second duration: **A-count parity queried by B** retains cumulative odd/even parity, **A-count multiple-of-3 queried by B** recognizes positive multiples of three, and **Odd A batch closed by B** reports parity since the preceding B and then clears it.
