...
- **Fitness for temporal targets** (`nv_evo/contracts.py`, `nv_evo/scoring.py`; `nv_evo/temporal.py` only runs trials and collects observations): the question is *"did the network implement the declared idea?"*, not *"did it copy an arbitrary trace?"*. A target owns a serializable `BehaviorContract` containing one or more reusable restrictions, and every backend passes normalized observations to `score_contract(...)` —
...
| File | Description |
|------|-------------|
| `app.py` | Single-window GUI: pick a model + target, inspect its executable Behavior Contract, evolve, inspect scored observations / growth / activity / genome, and drive circuits interactively |
...
| `diversity_ui.py` | "Diversity" tab — analyse a complete evaluated population, solved or failed: the four-level collapse funnel, fitness summary, and optional mutational-robustness histogram. Runs on a worker thread with progress and Stop; nervous/LUT only |
...
| `nv_evo/contracts.py` | Serializable behavior-contract and constraint data plus concise target-definition helpers |
| `nv_evo/scoring.py` | The single `score_contract` evaluator: truth tables, event correspondence, phase-invariant state, complete intervals, cadence, alignment, and reports |
| `nv_evo/temporal.py` | Temporal observation harness: trial running, contract-scored output placement, and `prepare_net` |
...
| `nv_evo/persistence.py` | Bounded-retention and SR curriculum target builders; their observations now use the same contract evaluator |
| `nv_evo/certification.py` | The held-out verdict rule (CERTIFIED / OVERFIT / SOLVED / PLATEAU) shared by the GUI and `reproduce.py` |
| `nv_evo/diversity.py` | Population diversity: the four-level collapse funnel (exact genotype → functional → phenotype → off-spec behaviour) plus mutational robustness. It works on unsuccessful populations too; once everyone solves, structural variety replaces the now-zero fitness spread |
...
used 100 generations, population 12, and two chromosomes. It was stopped at
204/230 matrix rows: 169 completed evolution runs, 35 declared-unsupported
pairs, zero errors, and 26 unstarted LUT rows. See
...
This is a small-population diagnostic, not evidence that contract v1 can only
reach the reported maxima.
